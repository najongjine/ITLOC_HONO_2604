...
io.on("connection", (socket) => {
    ...
    socket.on("join_room", async (data) => {
        ...
        `, [room.id, userId, receiverId]);
             ...
             */
            socket.join(roomId);
            console.log(`[socket.io] direct room joined. userId=${userId}, receiverId=${receiverId}, roomId=${roomId}`);
            /**
             * 8. 클라이언트에게 roomId 반환
             */
            socket.emit("joined_room", {
        ...
        `, [savedMessage.id, roomId, senderId]);
            ...
            if (client) {
                try {
                    ...
                    console.error("[socket.io] send_message rollback failed:", rollbackError);
                }
            }
